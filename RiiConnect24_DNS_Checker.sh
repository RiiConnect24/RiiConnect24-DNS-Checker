...
def script_start() {
	...
	Update_Activate=1
	offlinestorage=0
	FilesHostedOn=https://kcrpl.github.io/Patchers_Auto_Update/RiiConnect24_DNS_Checker

	current_rc24_server=164.132.44.106

	MainFolder=~/.temp/RiiConnect24_DNS_Checker
	TempStorage=~/.temp/RiiConnect24_DNS_Checker

	header = RiiConnect24 DNS Checker - (C) KcrPL v$version (Compiled on $last_build at $at)
...
def download_curl() {
	...
	echo '            ddmNNd:dNMMMMNMMMMMMMMMMMMMMMMMMMMMMMMMMs     You may be asked to use administrative privileges...'
...
:begin_main
curl
if not %errorlevel%==2 download_curl
clear
...
if %Update_Activate%==1 if %updateavailable%==1 update_notice
...
:whatsnew
clear
if not exist %TempStorage%\whatsnew.txt whatsnew_notexist
echo $header
echo ------------------------------------------------------------------------------------------------------------------------------
echo
...
curl -f -L -s -S --insecure "https://kcrpl.github.io/Patchers_Auto_Update/RiiConnect24Patcher/UPDATE/update_assistant.bat" --output "update_assistant.bat"
	...
	if not %temperrorlev%==0 error_updating
...
cd dig
if exist dig.exe if exist libbind9.dll if exist libcrypto-1_1-x64.dll if exist libdns.dll if exist libirs.dll if exist libisc.dll if exist libisccfg.dll if exist libuv.dll if exist libxml2.dll cd..&begin_main3
...
if not %temperrorlev%==0 error_download
cd..
begin_main3
:error_download
clear
